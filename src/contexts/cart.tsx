...
const CartProvider: React.FC = ({children}) => {
  ...
  useEffect(() => {
    async function loadProducts(): Promise<void> {
      const storagedProducts = await AsyncStorage.getItem('RNEcommerce:cart');
      if (storagedProducts) {
  ...
  const addToCart = useCallback(
    async (product) => {
      ...
      await AsyncStorage.setItem('RNEcommerce:cart', JSON.stringify(products));
    },
    [products],
  );
  const removeFromCart = useCallback(
    async (id) => {
      ...
      setProducts(list);
      await AsyncStorage.setItem('RNEcommerce:cart', JSON.stringify(list));
    },
    [products],
  );
  ...
  const value = React.useMemo(
    ...
    [products, addToCart, removeFromCart, increment, decrement, loading],
  );
